...
int key_gen_RS(secret_key_RS *secret_key_RS, public_key_RS *public_key_RS) {
    ...
    secret_key_RS->secret_key_content = NULL;
    ...
    secret_key_RS->secret_key_content = malloc(CRYPTO_SECRETKEYBYTES);

    raptor_keygen(public_key_tmp,
                  (unsigned char *)secret_key_RS->secret_key_content);

    public_key_RS->public_key_content = malloc(sizeof(int64_t) * DIM);
    memcpy(public_key_RS->public_key_content, public_key_tmp.h,
           sizeof(int64_t) * DIM);

    public_key_RS->public_key_length = sizeof(int64_t) * DIM;
    secret_key_RS->secret_key_length = CRYPTO_SECRETKEYBYTES;
...
int sign_RS(secret_key_RS *secret_key_signer, message *message,
    ...
    memcpy(signature_tmp[0].h, public_key_verifier->public_key_content,
           sizeof(int64_t) * DIM);
    memcpy(signature_tmp[1].h, public_key_signer->public_key_content,
           sizeof(int64_t) * DIM);

    if (raptor_sign(
            message->message_content, message->message_length, signature_tmp,
            (unsigned char *)secret_key_signer->secret_key_content, H) < 0) {
...
int verify_RS(signature *signature, message *message,
    ...
    if (raptor_verify(message->message_content, message->message_length, signature_tmp, H) < 0) {

        memcpy(signature_tmp[0].h, public_key_signer->public_key_content, sizeof(int64_t) * DIM);
        memcpy(signature_tmp[1].h, public_key_verifier->public_key_content, sizeof(int64_t) * DIM);
...
void free_key_pair_RS(secret_key_RS *secret_key_RS,
    ...
    free(public_key_RS->public_key_content);

    free((unsigned char *)secret_key_RS->secret_key_content);
